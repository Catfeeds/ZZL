            					...
            					<div class="table-responsive" style="margin: 0px 50px 50px 23px">
            						...
            						<div class="col-md-2">
										<button  type="button" class="btn btn-primary"
											style="margin-top: 34px" data-toggle="modal" onclick="showMaterialList()">增加</button>
									</div>
   		...
   		<script type="text/javascript">
        	function showMaterialList(){
        		...
        		newTH.innerHTML="<th>物料编号</th><th>物料名称</th><th>类型</th><th>成分</th><th>操作</th>";
					...
					jQuery.post("http://localhost:8080/ZZL/Bom/getMaterialList",params,function(json){
						var data = eval("("+json+")"); 
        	...
        	function addBom(materialCode){
        		...
        		var designID = $("input[name='designCode']").val();

        			var url=getRootPath()+"/Bom/";
	
				$.ajax({
					url:url+"addBom",
					data:{designCode:designID,materialCode:materialCode,volPerDesign:vol,materialPosition:position},
					type:"post",
					success:function(){
						alert("提交成功");
						 location.reload();
