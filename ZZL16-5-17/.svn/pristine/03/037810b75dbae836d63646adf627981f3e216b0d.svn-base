						...
						<div class="modal-body">
							...
							<form role="form" method="post"
								...
								<div class="table-responsive" style="margin: 0px 50px 50px 23px">
									<table class="table" frame="void" id="fuliao_list">
										...
										<tr id="0">
											...
											<td><input type="text" class="form-control" id="list-0-fuliao"></td>
											<td><label class="my_label">费用</label></td>
											<td><input type="text" class="form-control" id="list-0-cost"></td>
											<td><label class="my_label">元／米</label>
											</td>
											<td><label class="my_label">用量</label></td>
											<td><input type="text" class="form-control" id="list-0-counter"></td>
											<td><a id="addTable"
								...
								<table class="table" frame="void">
										...
										<td></td>
										<td></td>
										<td><button type="submit" class="btn btn-primary" style="float:right;" id="my_button">提交</button></td>
										</tr>
										</tbody>
									</table>
								</div>
...
<script>
	...
	$("#fuliao_list").append("<tr id='"+new_len+"'><td><label class='my_label'>辅料</label></td>"
			+"<td><input type='text' class='form-control' id='list-"+new_len+"-fuliao'></td>"
			+"<td><label class='my_label'>费用</label></td>"
			+"<td><input type='text' class='form-control' id='list-"+new_len+"-cost'></td>"
			+"<td><label class='my_label'>元／米</label></td>"
			+"<td><label class='my_label'>用量</label></td>"
			+"<td><input type='text' class='form-control'id='list-"+new_len+"-counter'></td>"
	...
	lastFuliao = $("#list-"+_len+"-fuliao").val();
	lastCost = $("#list-"+_len+"-cost").val();
	lastCounter = $("#list-"+_len+"-counter").val();
	for(var i=index+1;i<_len;i++){
		nextFuliao = $("#list-"+i+"-fuliao").val();
		nextCost = $("#list-"+i+"-cost").val();
		nextCounter = $("#list-"+i+"-counter").val();
		$("tr[id=\'" + i + "\']")
        			.replaceWith("<tr id='"+(i-1)+"'><td><label class='my_label'>辅料</label></td>"
        					+"<td><input type='text' class='form-control' id='list-"+(i-1)+"-fuliao' value='"+nextFuliao+"'></td>"
        					+"<td><label class='my_label'>费用</label></td>"
        					+"<td><input type='text' class='form-control' id='list-"+(i-1)+"-cost' value='"+nextCost+"'></td>"
        					+"<td><label class='my_label'>元／米</label></td>"
        					+"<td><label class='my_label'>用量</label></td>"
        					+"<td><input type='text' class='form-control'id='list-"+(i-1)+"-counter' value='"+nextCounter+"'></td>"
	...
	$("tr[id='"+_len+"']").replaceWith("<tr id='"+new_len+"'><td><label class='my_label'>辅料</label></td>"
			+"<td><input type='text' class='form-control' id='list-"+new_len+"-fuliao' value='"+lastFuliao+"'></td>"
			+"<td><label class='my_label'>费用</label></td>"
			+"<td><input type='text' class='form-control' id='list-"+new_len+"-cost' value='"+lastCost+"'></td>"
			+"<td><label class='my_label'>元／米</label></td>"
			+"<td><label class='my_label'>用量</label></td>"
			+"<td><input type='text' class='form-control'id='list-"+new_len+"-counter' value='"+lastCounter+"'></td>"
			+"<td><a id='addTable' class='btn btn-success' onclick='add_fuliao_list()'><i class='fa fa-plus'></i></a></td></tr>");
}
	</script>
</body>
